...
func (m *Memory) Grow(delta, _ int64) int64 {
	...
	if m.Buf == nil {
		m.allocate(uint64(m.Max) << 16) //nolint:gosec // page-count to byte-size, bounded by Max
	}
	...
	m.reallocate(uint64(newPages) << 16) //nolint:gosec // page-count to byte-size, bounded by Max
	return old
}

func (m *Memory) allocate(maxSz uint64) {
	// Round up to the page size.
	rnd := uint64(unix.Getpagesize() - 1) //nolint:gosec // page-size arithmetic
	res := (maxSz + rnd) &^ rnd
	...
	b, err := unix.Mmap(-1, 0, int(res), unix.PROT_NONE, unix.MAP_PRIVATE|unix.MAP_ANON) //nolint:gosec // mmap length, bounded by Max
	if err != nil {
		panic(err)
	}
	m.Buf = b[:0]
...
func (m *Memory) reallocate(size uint64) {
	com := uint64(m.com) //nolint:gosec // committed byte count
	...
	if com < size && size <= res {
		...
		m.com = int(newSz) //nolint:gosec // committed byte count
	}
	buf := m.Buf[:size]
